...
I_meat.center = center(I_meat_template);
I_meat.r= sqrt(sum(sum(I_meat_template)) / (pi*4/3));
I_circle_filter=zeros(I_class.Height,I_class.Width);
for i=1:I_class.Height
    for j=1:I_class.Width
        r_temp=sqrt(((i-I_meat.center.y)^2)/((4*I_meat.r/3)^2) + ((j-I_meat.center.x)^2)/((1*I_meat.r)^2));
        if((r_temp<=1))
...
I_class_out.center = I_meat.center;
I_class_out.r = I_meat.r;
